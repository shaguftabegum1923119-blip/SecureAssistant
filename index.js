...
fs.readdir(UPLOAD_DIR, (err, files) => {
  if (!err) files.forEach(f => { try{ fs.unlinkSync(path.join(UPLOAD_DIR, f)); }catch(e){} });
});
...
app.post('/api/payment/webhook', express.raw({type: 'application/json'}), async(req,res)=>{
  try{
    ...
    if(!secret) return res.status(500).json({error:"WEBHOOK_SECRET missing in Render ENV"});
    const expected = crypto.createHmac('sha256', secret).update(req.body).digest('hex');
    if(expected!== signature) return res.status(400).json({error:"Webhook signature FAIL blocked"});
    ...
    if(payment && payment.status==='captured' && db){
      ...
      if(snap.exists){
        if(snap.data().verified === true){
          return res.json({ok:true, alreadyVerified:true, message:"Already verified - No double credit"});
        }
      ...
      if(sSnap.exists){
         if(sSnap.data().verified === true){
           return res.json({ok:true, alreadyVerified:true, message:"Storage already added - No double credit"});
         }
         const data=sSnap.data();
         const extra=data.gb*1024*1024*1024;
         await db.collection('users').doc(data.userUid).update({storageLimit:admin.firestore.FieldValue.increment(extra),extraStorageGB:admin.firestore.FieldValue.increment(data.gb)});
         await db.collection('storage_payments').doc(orderId).update({paymentId:payment.id,verified:true,webhookVerified:true,verifiedAt:Date.now()});
      }
    }
    res.json({ok:true, corePromise: "verified"});
...
function getPriceFinal(count){ count = Math.max(1, Math.min(1000, parseInt(count)||1)); let per=calcRate(count); return { perCamera:per, monthly:count*per, yearly:count*per*10, count, offer:"Pay 10 months get 12 months", yearlySave:count*per*2 }; }
...
try{
  if(!process.env.RAZORPAY_KEY_ID ||!process.env.RAZORPAY_KEY_SECRET) throw new Error('Razorpay keys missing');
  razorpay=new Razorpay({key_id:process.env.RAZORPAY_KEY_ID,key_secret:process.env.RAZORPAY_KEY_SECRET});
  razorpayLive=true;
}catch(e){ razorpay=null; console.log("Razorpay OFF - Real payment only - Fake blocked:", e.message); }
...
const CATEGORIES=[
{id:1,name:"Home and Residential",subCategories:["House, Flat, Bungalow, Society Gate, Lift, Baby Room, Apartments, Houses, Villas, Residential Society, Slum Area, Small Lane, Farmhouse, Kitchen, Bedroom, Parking"],expertFeatures:["Intrusion after lock","Fall elders","Fire smoke","Scream audio","Person counting"],toolIds:[1,3,4,6,7,12,14,21]},
...
{id:3,name:"Office and Indoor",subCategories:["Corporate Office, IT Parks, Government Office, Co-Working, Software Company, IT Park, Call Center, BPO, Data Center, Server Room, Startup Office, CEO Cabin, Manager Cabin, Director Cabin, HR Office, Conference Hall, Meeting Room, Reception, Lobby, Cash Counter, Accounts, Office Building"],expertFeatures:["Staff attendance","Intrusion after hours","Loitering"],toolIds:[1,3,4,5,22]},
{id:4,name:"Lifestyle and Shopping",subCategories:["Malls, Supermarkets, Grocery Store, Salon, Gym, Restaurants, Hotels, Shopping Mall, Super Market, Showroom, Jewellery Shop, Bank, ATM"],expertFeatures:["Customer counting","Loyal recognition","Theft loitering","Daily report"],toolIds:[1,3,5,10,21]},
{id:5,name:"Transport and Travel Hubs",subCategories:["Air Transport: Airport, Runway, ATC, Helicopter Base, Cargo Airport, Rail Transport: Railway Station, Metro Station, Platform, Railway Yard, Loco Shed, Road Transport: Bus Station, Bus Depot, Van Stand, Jeep Stand, Auto Stand, Truck Terminal, Toll Plaza, Highway, Traffic Signal, Parking, Water Transport: Ship Port, Sea Port, Dockyard, Ferry Terminal, Private Hub: Car Showroom, Bike Showroom, Driving School, Petrol Pump, EV Charging, Bus Stand, Railway Station, Airport, Parking, Petrol Pump"],expertFeatures:["Vehicle detection","Number plate ANPR","Crowd","Unattended baggage"],toolIds:[2,9,10,11]},
{id:6,name:"Roads and Streets",subCategories:["National Highways, City Roads, Small Lanes, Toll Tax, Highway, Traffic Signal, Toll Plaza, Bus Depot, Truck Terminal, Auto Stand"],expertFeatures:["Vehicle counting","Number plate ANPR","Accident fall"],toolIds:[2,9,6]},
{id:7,name:"Industrial Factories and Mines",subCategories:["Factories, Warehouses, Mines, Construction Sites, Factory, Warehouse, Storage Warehouse, Cold Storage, Production Floor, Loading Area"],expertFeatures:["Fire smoke alert","Person safety","Vehicle entry","Intrusion"],toolIds:[4,6,7,9]},
...
{id:9,name:"Religious Places",subCategories:["Mosque, Temple, Church, Gurudwara, Shrine, Temple"],expertFeatures:["Crowd management","Unattended bag","Person counting","Live location"],toolIds:[3,10,11,18]},
...
{id:11,name:"Agriculture and Farms",subCategories:["Farms, Poultry Farms, Dairy Farms, Wholesale Market, Farm, Market Yard"],expertFeatures:["Intrusion night","Vehicle detection","Person counting","Fire alert"],toolIds:[4,7,9,14]},
...
app.post('/api/auth/forgot-password', async(req,res)=>{
  try{
    ...
    res.json({success:true, message:"Password reset link generated - Send via email", resetLink:link, corePromise:CORE_PROMISE});
  }catch(e){ res.status(400).json({error:e.message}); }
});
...
app.post('/api/payment/calculate',auth,(req,res)=>{
  ...
  if(!isValidCount(count)) return res.status(400).json({error:"cameraCount 1-1000 only"});
  res.json(getPriceFinal(count));
});
app.post('/api/payment/generate-qr',auth,async(req,res)=>{
  try{
    if(!razorpayLive ||!razorpay) return res.status(500).json({error:"Razorpay not configured - No fake order allowed - Add RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET in Render ENV"});
    let {cameraCount,totalCameraCount,locations,timezone}=req.body;
    let count=totalCameraCount||cameraCount||1;
    if(locations&&Array.isArray(locations)) count=locations.reduce((s,l)=>s+(l.cameraCount||0),0);
    if(!isValidCount(count)) return res.status(400).json({error:"cameraCount 1-1000 only"});
    let per=calcRate(count); let amount=count*per*100;
    let order=await razorpay.orders.create({amount,currency:'INR',receipt:'rec_'+Date.now()});
    if(firestoreLive){ const wt=getWorldTime(timezone); await db.collection('payments').doc(order.id).set({userUid:req.user.uid,userPhone:req.user.phone,count,total:count*per,orderId:order.id,created:Date.now(),createdAtExact:wt.utcExact,createdLocal:wt.local,verified:false,rateCard:RATE_CARD}); }
    res.json({orderId:order.id,keyId:process.env.RAZORPAY_KEY_ID,count,total:count*per,rateCard:RATE_CARD,locations,corePromise:CORE_PROMISE,firestoreLive});
  }catch(e){ res.status(500).json({error:e.message}); }
});
app.post('/api/payment/verify',auth,async(req,res)=>{
  try{
    let {razorpay_order_id,razorpay_payment_id,razorpay_signature,timezone}=req.body;
    if(!razorpay_order_id ||!razorpay_payment_id ||!razorpay_signature) return res.status(400).json({error:"All razorpay fields required"});
    let body=razorpay_order_id+"|"+razorpay_payment_id;
    let expected=crypto.createHmac('sha256',process.env.RAZORPAY_KEY_SECRET).update(body).digest('hex');
    if(expected.length!==(razorpay_signature||'').length) return res.json({success:false,error:"Signature FAIL blocked"});
    const ok=crypto.timingSafeEqual(Buffer.from(expected),Buffer.from(razorpay_signature));
    if(ok){ if(firestoreLive){ const wt=getWorldTime(timezone); await db.collection('payments').doc(razorpay_order_id).update({paymentId:razorpay_payment_id,verified:true,verifiedAt:Date.now(),verifiedAtExact:wt.utcExact,verifiedLocal:wt.local}); } res.json({success:true,message:"Signature Verified",corePromise:CORE_PROMISE}); } else res.json({success:false,error:"Signature FAIL"});
...
app.get('/api/payment/history',auth,async(req,res)=>{
  try{
    if(!firestoreLive) return res.status(500).json({error:"Firestore not live"});
    let history=[]; let sHistory=[];
    try{
      const snap=await db.collection('payments').where('userUid','==',req.user.uid).orderBy('created','desc').limit(50).get();
      history=snap.docs.map(d=>d.data());
    }catch(e){
      const snap=await db.collection('payments').where('userUid','==',req.user.uid).limit(50).get();
      history=snap.docs.map(d=>d.data()).sort((a,b)=>b.created-a.created);
    }
    try{
      const sSnap=await db.collection('storage_payments').where('userUid','==',req.user.uid).orderBy('created','desc').limit(50).get();
      sHistory=sSnap.docs.map(d=>d.data());
    }catch(e){
      const sSnap=await db.collection('storage_payments').where('userUid','==',req.user.uid).limit(50).get();
      sHistory=sSnap.docs.map(d=>d.data()).sort((a,b)=>b.created-a.created);
    }
...
app.post('/api/storage/buy-extra',auth,async(req,res)=>{
  try{
    if(!razorpayLive ||!razorpay) return res.status(500).json({error:"Razorpay not configured - No fake order"});
    const {planKey,timezone}=req.body; const plan=STORAGE_PLANS[planKey];
    ...
    if(firestoreLive){ const wt=getWorldTime(timezone); await db.collection('storage_payments').doc(order.id).set({userUid:req.user.uid,planKey,gb:plan.gb,price:plan.price,orderId:order.id,created:Date.now(),createdAtExact:wt.utcExact,createdLocal:wt.local,verified:false}); }
...
app.post('/api/storage/buy-custom',auth,async(req,res)=>{
  try{
    if(!razorpayLive ||!razorpay) return res.status(500).json({error:"Razorpay not configured - No fake order"});
    let {customGB,timezone}=req.body; customGB=parseInt(customGB);
    ...
    if(firestoreLive){ const wt=getWorldTime(timezone); await db.collection('storage_payments').doc(order.id).set({userUid:req.user.uid,planKey:"custom_"+customGB,gb:customGB,price,orderId:order.id,created:Date.now(),createdAtExact:wt.utcExact,createdLocal:wt.local,verified:false}); }
...
app.post('/api/storage/verify-extra',auth,async(req,res)=>{
  try{
    let {razorpay_order_id,razorpay_payment_id,razorpay_signature,timezone}=req.body;
    if(!razorpay_order_id ||!razorpay_payment_id ||!razorpay_signature) return res.status(400).json({error:"All fields required"});
    let body=razorpay_order_id+"|"+razorpay_payment_id;
    let expected=crypto.createHmac('sha256',process.env.RAZORPAY_KEY_SECRET).update(body).digest('hex');
...
app.get('/api/chat/history',auth,async(req,res)=>{
  if(!firestoreLive) return res.status(500).json({error:"Firestore not live"});
  try{
    const snap=await db.collection('chats').where('userUid','==',req.user.uid).orderBy('created','desc').limit(50).get();
    ...
    const snap=await db.collection('chats').where('userUid','==',req.user.uid).limit(50).get();
    const list=snap.docs.map(d=>d.data()).sort((a,b)=>a.created-b.created);
    res.json({count:list.length,chats:list,rateCard:RATE_CARD,corePromise:CORE_PROMISE, note:"Index building"});
  }
});
app.post('/api/video/upload-analyze',auth,upload.single('video'),async(req,res)=>{
  if(!firestoreLive) return res.status(500).json({error:"Firestore not live"});
  const uSnap=await db.collection('users').doc(req.user.uid).get(); const uData=uSnap.data()||{storageUsed:0,storageLimit:BASE_BYTES}; const used=uData.storageUsed||0; const limit=uData.storageLimit||BASE_BYTES;
  if(used>=limit) return res.status(403).json({error:"Storage Full",plans:STORAGE_PLANS,rateCard:RATE_CARD});
  const file=req.file; if(!file) return res.status(400).json({error:"video file required"});
  try{
    const buf=fs.readFileSync(file.path); const sha256=crypto.createHash('sha256').update(buf).digest('hex'); const wt=getWorldTime(req.body.timezone||uData.timezone);
    const capsule={id:"cap_"+wt.ts,fileName:file.originalname,size:file.size,sha256,time:wt.ts,createdAtExact:wt.utcExact,localTime:wt.local,timezone:wt.tz,realClickProof:true};
    await db.collection('users').doc(req.user.uid).update({capsules:admin.firestore.FieldValue.arrayUnion(capsule),storageUsed:admin.firestore.FieldValue.increment(file.size)});
    await db.collection('incidents').add({userUid:req.user.uid,capsule,created:wt.ts,createdAtExact:wt.utcExact,corePromise:CORE_PROMISE});
    res.json({success:true,capsuleId:capsule.id,sha256,rateCard:RATE_CARD,corePromise:CORE_PROMISE});
  }finally{ try{fs.unlinkSync(file.path);}catch(e){} }
});
app.post('/api/alert/send',auth,async(req,res)=>{
  try{
    ...
    const uSnap=await db.collection('users').doc(req.user.uid).get(); const user=uSnap.data()||{}; const wt=getWorldTime(timezone||user.timezone);
    ...
    await db.collection('alerts').add(alertData); res.json({success:true,alert:alertData,rateCard:RATE_CARD});
  }catch(e){ res.status(500).json({error:e.message}); }
});
app.get('/api/alert/list',auth,async(req,res)=>{
  if(!firestoreLive) return res.status(500).json({error:"Firestore not live"});
  try{
    const snap=await db.collection('alerts').where('userUid','==',req.user.uid).orderBy('created','desc').limit(50).get();
    const list=snap.docs.map(d=>d.data());
    res.json({count:list.length,alerts:list,rateCard:RATE_CARD,corePromise:CORE_PROMISE});
  }catch(e){
    const snap=await db.collection('alerts').where('userUid','==',req.user.uid).limit(50).get();
...
app.get('/api/team/list',auth,async(req,res)=>{
  if(!firestoreLive) return res.status(500).json({error:"Firestore not live"});
  const h=await db.collection('team_holders').where('ownerUid','==',req.user.uid).limit(3).get();
  const s=await db.collection('team_staff').where('ownerUid','==',req.user.uid).limit(100).get();
...
app.post('/api/person/search-where',auth,async(req,res)=>{
  ...
  let lastSeen=null;
  try{
    const alertSnap=await db.collection('alerts').where('userUid','==',req.user.uid).orderBy('created','desc').limit(50).get();
    lastSeen=alertSnap.docs.map(d=>d.data()).filter(a=>(a.message||"").toLowerCase().includes((personName||"").toLowerCase())).sort((a,b)=>b.created-a.created)[0];
  }catch(e){
    const alertSnap=await db.collection('alerts').where('userUid','==',req.user.uid).limit(50).get();
    lastSeen=alertSnap.docs.map(d=>d.data()).filter(a=>(a.message||"").toLowerCase().includes((personName||"").toLowerCase())).sort((a,b)=>b.created-a.created)[0];
  }
  if(!lastSeen){
    try{
      const incSnap=await db.collection('incidents').where('userUid','==',req.user.uid).orderBy('created','desc').limit(50).get();
      let f=incSnap.docs.map(d=>d.data()).filter(a=>(a.capsule?.fileName||"").toLowerCase().includes((personName||"").toLowerCase())).sort((a,b)=>b.created-a.created)[0];
...
app.post('/api/sms/send',auth,async(req,res)=>{
  try{
    ...
    if(!process.env.TWILIO_SID ||!process.env.TWILIO_TOKEN ||!process.env.TWILIO_PHONE){
      console.log(`SMS LOG ONLY (Add TWILIO_SID): ${phone}: ${message}`);
      if(firestoreLive) await db.collection('sms_logs').add({ownerUid:req.user.uid,phone,message,created:Date.now(),mode:"log_only_no_twilio_env",corePromise:CORE_PROMISE});
      return res.json({success:true, mode:"logged_only - Add TWILIO_SID, TWILIO_TOKEN, TWILIO_PHONE in Render ENV for real SMS", message:"SMS logged"});
    }
    const twilio = require('twilio')(process.env.TWILIO_SID, process.env.TWILIO_TOKEN);
    const result = await twilio.messages.create({body:message, from:process.env.TWILIO_PHONE, to:phone});
    if(firestoreLive) await db.collection('sms_logs').add({ownerUid:req.user.uid,phone,message,created:Date.now(),mode:"twilio_real",sid:result.sid,corePromise:CORE_PROMISE});
    res.json({success:true, mode:"twilio_real", sid:result.sid, message:"SMS sent real - "+CORE_PROMISE});
  }catch(e){ res.status(500).json({error:e.message}); }
});
